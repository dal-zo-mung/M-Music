...
async function fetchAllSongsList() {
  const songs = await fetchJson(`${API_URL}/songs`);
...
async function fetchSongsByQuery(query) {
  ...
  const songs = await fetchJson(`${API_URL}/songs/search/${encodeURIComponent(trimmedQuery)}?limit=50`);
  return Array.isArray(songs) ? songs : [];
}
...
function getSongPageUrl(songId, query = activeSearchQuery) {
  const url = new URL('./lyrics.html', window.location.href);
...
function navigateToSearchPage(query) {
  ...
  window.location.href = getSearchPageUrl(trimmedQuery);
}

function updateSearchPageCopy(query, total) {
  if (!isSearchPage) return;

  const trimmedQuery = String(query || '').trim();

  if (!trimmedQuery) {
    if (els.searchPageTitle) els.searchPageTitle.textContent = 'Find the song you want to open';
    if (els.searchSummary) els.searchSummary.textContent = 'Search by song title or artist to see matching results.';
    return;
  }

  if (els.searchPageTitle) {
    els.searchPageTitle.textContent = `Results for "${trimmedQuery}"`;
  }

  if (els.searchSummary) {
    const noun = total === 1 ? 'match' : 'matches';
    els.searchSummary.textContent = `${total} ${noun} found. Select a song to open its detail page.`;
  }
}
...
function renderEmptySearchState(message) {
  ...
  emptyState.className = 'search-result empty-state';
...
async function loadAllSongs() {
  try {
    setLoading(true);
    allSongs = await fetchAllSongsList();
...
async function loadSongById(id) {
  try {
    setLoading(true);
    const song = await fetchJson(`${API_URL}/songs/${id}`);
    renderSong(song);
  } catch (error) {
...
async function searchSongs(query) {
  ...
  if (!trimmedQuery) {
    updateSearchPageCopy('', 0);
    renderEmptySearchState('Search for a song or artist to see matching results.');
    return [];
  }

  try {
    const songs = await fetchSongsByQuery(trimmedQuery);
    updateSearchPageCopy(trimmedQuery, songs.length);
    renderSearchResults(songs);
    return songs;
  } catch (error) {
    console.error('searchSongs error:', error);
    updateSearchPageCopy(trimmedQuery, 0);
    renderEmptySearchState('Unable to load search results right now.');
    return [];
  }
}
...
function renderSearchResults(songs) {
  ...
  if (!songs || songs.length === 0) {
    renderEmptySearchState(`No results found for "${activeSearchQuery || 'your search'}".`);
    return;
  }
  ...
  songs.forEach((song) => {
    ...
    result.addEventListener('click', () => {
      window.location.href = getSongPageUrl(song._id, activeSearchQuery);
    });
...
function setupEventListeners() {
  els.searchbt.addEventListener('click', () => {
    navigateToSearchPage(els.searchbox.value);
  });
...
function setupSearchPage() {
  const runSearch = async (query) => {
    const trimmedQuery = String(query || '').trim();
    const nextUrl = getSearchPageUrl(trimmedQuery);
    window.history.replaceState({}, '', nextUrl);
    await searchSongs(trimmedQuery);
  };
...
(async function init() {
  ...
  if (isSearchPage) {
    ...
    if (initialQuery) {
      ...
      await searchSongs(initialQuery);
    } else {
      updateSearchPageCopy('', 0);
      renderEmptySearchState('Search for a song or artist to see matching results.');
    }
  ...
  const selectedSongId = params.get('id');
  ...
  if (selectedSongId) {
    try {
      setLoading(true);
      allSongs = initialQuery ? await fetchSongsByQuery(initialQuery) : await fetchAllSongsList();

      if (allSongs.length > 0) {
        const selectedIndex = allSongs.findIndex((item) => item._id === selectedSongId);
        if (selectedIndex >= 0) {
          await loadSongByIndex(selectedIndex);
          return;
        }
      }

      await loadSongById(selectedSongId);
    } catch (error) {
      console.error('Failed to initialize selected song:', error);
      await loadSongById(selectedSongId);
    } finally {
      setLoading(false);
    }
  ...
  if (initialQuery) {
    window.location.replace(getSearchPageUrl(initialQuery));
    return;
  }
